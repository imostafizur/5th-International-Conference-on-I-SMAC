...
bool Gsender::Send(const String &to, const String &message)
{
  ...
  int inputLen = strlen(EMAIL_LOGIN);
  
  int encodedLen = base64_enc_len(inputLen);
  char encoded_LOGIN[encodedLen];
  
  base64_encode(encoded_LOGIN, (char*)EMAIL_LOGIN, inputLen); 
  
  encoded_LOGIN[strlen(encoded_LOGIN)-1] = '=';
  client.println((const char*)encoded_LOGIN);
  AwaitSMTPResponse(client);

#if defined(GS_SERIAL_LOG_2)
  Serial.println("EMAILBASE64_PASSWORD:");
#endif
  inputLen = strlen(EMAIL_PASSWORD);
  
  encodedLen = base64_enc_len(inputLen);
  char encoded_PASSWORD[encodedLen];
  
  base64_encode(encoded_PASSWORD, (char*)EMAIL_PASSWORD, inputLen); 
  
  
  encoded_PASSWORD[strlen(encoded_PASSWORD)-1] = '=';
  client.println((const char*)encoded_PASSWORD);
